...
def get_links(search='', pages=0):
    ...
    for i in range(0, int(pages)):
        # Use url
        url = 'https://www.mycareersfuture.sg/search?search={}&sortBy=new_posting_date&page={}'.format(url_frag, i)
        print('Getting links from {} ...'.format(url))
        
        # Get page and wait 1 second
        driver.get(url)
        ...
        for i in soup.find_all('a', {'class':'bg-white'}):
            ...
            # Save to new txt file every 500 links
            if len(links) % 500 == 0:
...
def mycareersfuture_scraper():
    ...
    links = get_links(search=search, pages=pages)
...
details.to_csv('mycareersfuture.csv')

# Put all scraped links into txt file
with open('mycareersfuture_links.txt', 'wb') as fp:
    pickle.dump(links, fp)
